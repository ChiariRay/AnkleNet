...
def parse_yaml(opt_path:str):
    yaml_file = open(opt_path, "r", encoding="utf-8")
    file_data = yaml_file.read()
    yaml_file.close()

    opt = yaml.load(file_data, Loader=yaml.FullLoader)

    return opt


def parse_json(opt_path, is_train=True):
